...
public class NewMsgDbHelper {             //新消息条数
	...
	private class SqlLiteHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {   //MsgId对聊天来说即对方id
			String sql = "CREATE TABLE  IF NOT EXISTS newMsg"
						...
						"i_field1 INTEGER, t_field1 text, Iswork INTEGER, serialnumber text, link text, " +
						"issubmit INTEGER, parms text, Datas text, msgUrl text)";
			db.execSQL(sql);
		}
	...
	public void saveNewMsg(String msgId){    //如果存在，则+1，不存在则插入
		...
		if(nowCount==0){
			...
			values.put("whosMsg", Constants.XMPP_USERNAME);
			...
			db.update(helper.DB_NAME, values, " msgId=? and whosMsg=?", 
					new String[]{msgId,Constants.XMPP_USERNAME});
		}
	}
	...
	public void delNewMsg(String msgId){
		db.delete(helper.DB_NAME, " msgId=? and whosMsg=?", new String[]{msgId,Constants.XMPP_USERNAME});
	}
	...
	public int getMsgCount(String msgId){
		...
		Cursor cursor = db.rawQuery(sql, new String[]{msgId,Constants.XMPP_USERNAME});
	...
	public int getMsgCount(){   
		...
		Cursor cursor = db.rawQuery(sql, new String[]{Constants.XMPP_USERNAME});
		while(cursor.moveToNext()){
			count = cursor.getInt(0);
		}
		cursor.close();
